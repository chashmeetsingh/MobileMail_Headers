//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import "NSObject.h"

#import "MFDiagnosticsGenerator.h"
#import "MFPowerObserver.h"
#import "MessageBodyLoader.h"

@class MFActivityMonitor, MFCancelationToken, MFInvocationQueue, MFMailMessage, MFMailMessageLibrary, MailAccount, NSCountedSet, NSDate, NSMutableArray, NSMutableDictionary, NSMutableSet, NSObject<OS_dispatch_queue>, NSObject<OS_dispatch_semaphore>, NSString;

@interface MessageBodyLoader : NSObject <MFDiagnosticsGenerator, MessageBodyLoader, MFPowerObserver>
{
    NSMutableArray *_clients;
    NSMutableArray *_newMessages;
    NSMutableSet *_newMessageSet;
    NSCountedSet *_newMessageMailboxQuota;
    NSMutableArray *_userViewingMessages;
    MFActivityMonitor *_currentMonitor;
    NSCountedSet *_watchedUids;
    NSObject<OS_dispatch_queue> *_dispatchQueue;
    MFInvocationQueue *_workQueue;
    MFCancelationToken *_cancelationToken;
    MFMailMessageLibrary *_library;
    NSObject<OS_dispatch_semaphore> *_waitReply;
    int _noNetworkFetching;
    int _isPaused;
    NSDate *_resumeTime;
    unsigned int _isRunning:1;
    unsigned int _isWaitingForTask:1;
    unsigned int _shouldProcessQueues:1;
    unsigned int _holdingPowerAssertion:1;
    NSMutableDictionary *_bytesPerAccount;
    MFMailMessage *_currentMessage;
    MailAccount *_account;
}

+ (void)accountsDidChange;
+ (void)applicationWillResume;
+ (void)applicationWillSuspend;
+ (id)copySummaryForMessage:(id)arg1 downloadIfNecessary:(_Bool)arg2;
+ (void)resume;
+ (void)pause;
+ (id)loaderForAccount:(id)arg1;
+ (void)_setSharedLoaderForTesting:(id)arg1 account:(id)arg2;
+ (void)initialize;
@property(retain, nonatomic) MailAccount *account; // @synthesize account=_account;
- (void)_releasePowerAssertion;
- (void)_retainPowerAssertion;
- (void)_clearResumeTime;
- (void)_cancelSleepIfNeeded;
- (void)systemDidWake;
- (void)systemWillSleep;
- (void)assertionDidExpire;
- (void)_logStats:(_Bool)arg1;
- (void)_recordStats:(id)arg1;
- (void)_applicationDidEnterBackground:(id)arg1;
- (void)_applicationDidBecomeActive:(id)arg1;
- (void)applicationWillResume;
- (void)applicationWillSuspend;
- (oneway void)disableNetworkFetching;
- (oneway void)enableNetworkFetching;
- (oneway void)resume;
- (oneway void)pause;
@property(readonly) _Bool isPaused;
- (void)_start;
- (_Bool)_isRunning;
- (void)_setIsRunning:(_Bool)arg1;
- (void)_blockingMessage;
- (void)_waitUntilNotRunning;
- (oneway void)userStoppedViewingMessages;
- (oneway void)userViewingMessages:(id)arg1;
- (oneway void)stopAddingNewMessagesForStore:(id)arg1;
- (oneway void)beginAddingNewMessagesForStore:(id)arg1;
- (_Bool)_nts_isProcessingMessage:(id)arg1;
- (_Bool)isProcessingMessage:(id)arg1;
- (oneway void)addMessages:(id)arg1;
- (oneway void)addMessages_nts:(id)arg1;
- (oneway void)addMessage:(id)arg1;
- (void)_nts_sortClients;
- (void)_nts_removeClient:(id)arg1;
- (void)_nts_insertClient:(id)arg1;
- (oneway void)removeSingleMessageClient:(id)arg1;
- (oneway void)addSingleMessageClient:(id)arg1;
- (void)_messageFlagsChanged:(id)arg1;
- (void)_messagesWillBeCompacted:(id)arg1;
- (void)_removeViewingMessages:(id)arg1;
- (void)_removeNewMessages:(id)arg1;
- (void)_removeNewMessages_nts:(id)arg1;
- (void)_messagesAdded:(id)arg1;
- (void)_getNextClientOrMessageFinished:(id)arg1;
- (void)_messageLoadFinished:(id)arg1;
- (void)_clientLoadFinished:(id)arg1;
- (void)_finishedCullingMessageList:(id)arg1;
- (void)_getNextClientOrMessage;
- (void)_tryProcessingQueues;
- (id)copyDiagnosticInformation;
- (_Bool)networkFetchingAllowed;
- (void)dealloc;
- (oneway void)startup;
- (id)initWithLibrary:(id)arg1;
- (id)init;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

